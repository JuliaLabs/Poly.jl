...
function mul_optimized_thread(A::Array{T,2}, B::Array{T,2}, C::Array{T,2}) where {T}
    N = size(C, 1)
    M = size(C, 2)
    R = size(A, 2)
    TILE_DIM = 64

    @threads for gj = 0:TILE_DIM:M-1
        @simd for t = 0:TILE_DIM:R-1
            @simd for gi = 0:TILE_DIM:N-1
                @simd for j = 1:TILE_DIM
                    @simd for k = 1:TILE_DIM
...
function mul_poly_thread(A::Array{T,2}, B::Array{T,2}, C::Array{T,2}) where {T}
    N = size(C, 1)
    M = size(C, 2)
    R = size(A, 2)
...
function mul_poly_rt_thread(A::Array{T,2}, B::Array{T,2}, C::Array{T,2}) where {T}
    N = size(C, 1)
    M = size(C, 2)
    R = size(A, 2)
